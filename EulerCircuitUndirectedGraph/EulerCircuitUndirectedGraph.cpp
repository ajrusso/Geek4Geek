...
denoting the graph. Find that there exists the Euler circuit or not. Return 1 if there exist  alteast one
eulerian path else 0.
...
Your Task:
You don't need to read or print anything. Your task is to complete the function isEularCircuitExist() 
which takes v and adjacency list adj[] as input parameter and returns boolean value 1 if Eular circuit 
...
bool isEularCircuitExit(int, vector<int>[]);
...
int main() {
    ...
    bool result = isEularCircuitExit(v, adj);

    cout << "Eulerian path " << (result ? "is" : "is NOT") << " found." << endl;                        
}
