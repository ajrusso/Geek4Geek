...
User Task:
 Your task is to complete the function countPairs() which takes 4 arguments mat1, mat2, n, x, and returns
 the count of pairs whose sum equals to x. You don't need to take any input or print anything.
...
int countPairs(vector<vector<int>>&, vector<vector<int>>&, int, int);

int main() {
    vector<vector<int>> mat1 = {{1,2},
                                {3,4}};
    vector<vector<int>> mat2 = {{4,5},
                                {6,7}};
    int n = 2;
    int x = 10;

    int pairsSum = countPairs(mat1, mat2, n, x);

    cout << "Pairs Sum = " << pairsSum << endl;
}
...
int countPairs(vector<vector<int>> &mat1, vector<vector<int>> &mat2, int n, int x) {
    ...
    for(const auto& v1 : mat1) {
        for(int i : v1) {
            s.insert(i);       
        }
    }
            
    for(const auto& v2 : mat2) {
        for(int i : v2) {
            if(s.find(x-i) != s.end())
                count++;
        }
